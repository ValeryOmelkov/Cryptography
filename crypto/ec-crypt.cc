...
//			cf(R_before, round_key, R_after);
	...
	static uint32_t gost_34_12_64_cipher(const uint32_t 	a,
										 const uint32_t 	k) {
	...
	static void gost_34_12_64_key_expand(const uint8_t	*key,
										 uint32_t 		*expand_key) {
	...
	void crypt_gost_34_12_64(const uint8_t	*src_block,
							 const uint8_t 	*key,
							 bool 			reverse_key,
							 uint8_t 		*dst_block) {
